...
## Vault Map (6 рабочих папок + 7. Вики)

| Папка | Что внутри | Когда читать |
|---|---|---|
| `1. Проект/` | Обзор, стек, архитектура | новый контекст, архитектурные задачи |
| `2. Модули/` | По файлу на модуль (YAML-свойства), **тех-доки для разработчика** | **ОБЯЗАТЕЛЬНО** перед правкой модуля |
...
| `7. Вики/` | **Продуктовая база знаний для людей** (A.Сайт / B.User Guide / C.Сценарии / D.Тех / E.Связи / F.Бренд) | онбординг, маркетинг, обучение юзеров; обновлять при релизе фичи / смене UX. Правила — `references/wiki-guide.md` |
...
| Фича доступна юзерам / смена UX | — | `7. Вики/B` (+ `A` если влияет на позиционирование) + `00 — Индекс` |
| Смена цены/позиционирования | `5. Планы` | `7. Вики/A` |
...
- **Параллельный чат на одном проекте:** STATE общий → перед работой ПЕРЕПИШИ `→ Следующий шаг:` под свою задачу. Иначе после компакта подхватишь чужой шаг (как у соседней сессии). parallel-guard предупредит на старте о свежем чужом апдейте, но дисциплина «свой → шаг» — на тебе.
...
- `references/wiki-guide.md` — правила наполнения и обновления `7. Вики/` (продуктовая база знаний).
