...
- **Матрица-роутер** — явные правила: *какую папку vault читать до и куда писать после* каждого вида работы (больше не угадываем).
- **Единый якорь восстановления** — ровно одна строка `→ Следующий шаг:`; хуки возобновляют работу с неё после сжатия.
- **Детерминированный `compression_count`** — увеличивает PreCompact хук, а не Claude.
- **Защита от параллельных сессий** — предупреждает, если SESSION_STATE только что трогала другая сессия; один канонический STATE, без дублей.
- **Полностью обезличен** — реюзабельный скелет; специфика проекта живёт в `.claude/brain.conf` + project CLAUDE.md.
...
Каждый проект получает папку vault с 6 рабочими директориями + опциональная продуктовая вики (v3.1):

```
Obsidian Vault/MyProject/
├── 1. Проект/              # Обзор, стек, архитектура
├── 2. Модули/              # Один файл на модуль (тех-доки для разработчика)
...
└── 7. Вики/                # Продуктовая база знаний для людей (маркетинг/юзеры/поддержка)
```

**Матрица-роутер** (`references/routing-matrix.md`) явно говорит, какую папку читать до и куда писать после каждого вида работы. Правила `7. Вики/` — `references/wiki-guide.md`.
